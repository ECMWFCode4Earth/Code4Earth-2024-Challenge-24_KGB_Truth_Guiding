...
#    tables = camelot.read_pdf(file_name)
    ...
    for pageNumber, page in enumerate(doc.pages(), start=1):
        ...
        with open(os.path.join(text_dir, f"output_{pageNumber}.txt"), "w") as out:
            ...
            whole_content.append(text)  # add text to a list of text presenting of each page 
    ...
    with open(os.path.join(text_dir, "content.txt"), "r") as file:
        ...
        figure_descriptions = []
        table_descriptions = []
        figure_pattern = re.compile(r"^Figure \d+:")    # This signifies the start of a figure
        table_pattern = re.compile(r"^Table \d+:")    # This signifies the start of a table
        num_lines = len(lines)
        curr_line = 0
            ...
            if figure_pattern.match(line):
                while True:
                    ...
                    if inloop_line.endswith('.\n') or len(inloop_line.split()) > 7 :
                        ...
                        break
                description_range = 0
                while description_range < 10 and curr_line + description_range < num_lines:     # We add number 10 for safeguard so this don't run forever if the second condition doesn't match
                    ...
                    if inloop_line.endswith('.\n'):    # if a line reach ".\n" it means that it reach the end of description
                        ...
                        description = " ".join([line.rstrip("\n") for line in lines[curr_line:curr_line+description_range]])    # concatenate the desciption
                        figure_descriptions.append(description)
                        clean_lines = clean_lines + lines[curr_line:curr_line+description_range]
                        
                        break
                curr_line = curr_line + description_range    # jump the line to the text after the figure
                ...
                while curr_line < num_lines:         # skip all the line until reach the the condition below
