...
def get_page_url_from_page_number(page_number, source):
    """
    Return page url from page number
    ...
    Return:
        url_list (list): Page URLs to search inside and download.
    ...
    if re.match(r"^\d+$", page_number):  # match an integer
        ...
        r"^\[\s*[1-9]\d*\s*,\s*[1-9]\d*\s*\]$", page_number
    ):  # match form [a,b]
...
@click.option(
    ...
    required=True,
    type=click.Choice(
        ["memoranda", "report", "ERA", "esa-eumetsat", "eumetsat-ecmwf"],
        case_sensitive=False,
    ),
    help="Choice the source to crawl from",
)
@click.option(
    "--pages",
    required=True,
